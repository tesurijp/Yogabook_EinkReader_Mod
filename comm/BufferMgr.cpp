...
//根据offset获取EI_BUFFER
EI_BUFFER* CBufferMgr::GetDrawBuffer(ULONG nulOffset)
{
	EI_BUFFER* lpRet = NULL;
	do
	{
		if (mpBufferBase == NULL)
			break;
		//lpRet = (EI_BUFFER*)(mpBufferBase + nulOffset + EAC_MSG_BUFFER_SIZE);
		lpRet = (EI_BUFFER*)(mpBufferBase + nulOffset);
	} while (false);
...
//返回一次绘制所需内存大小
ULONG CBufferMgr::GetOnceDrawBufferSize()
{
	return mulBlockSize + sizeof(EI_BUFFER) + 1;
}
